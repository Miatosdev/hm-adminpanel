...
else
{
    ...
    else
    {
        if($refercode != '')
	   {
		...
		$table="tbl_user";
  $field_values=array("coin","name","email","mobile","rdate","password","ccode","refercode","wallet","code","gender","lats","longs","birth_date","search_preference","radius_search","relation_goal","interest","language","religion","other_pic","profile_bio");
  $_POST_values=array("0","$fname","$email","$mobile","$timestamp","$password","$ccode","$refercode","$fin","$prentcode","$gender","$lats","$longs","$birth_date","$search_preference","$radius_search","$relation_goal","$interest","$language","$religion","$multifile","$profile_bio");
 ...
 $c = $dating->query("select * from tbl_user where id=".$check."")->fetch_assoc();
	    ...
	    else 
	   {
        ...
        $multifile = implode('$;', $uploadedFiles);
		   ...
		   $table="tbl_user";
  $field_values=array("coin","name","mobile","rdate","password","ccode","code","email","gender","lats","longs","birth_date","search_preference","radius_search","relation_goal","interest","language","religion","other_pic","profile_bio");
  $_POST_values=array("0","$fname","$mobile","$timestamp","$password","$ccode","$prentcode","$email","$gender","$lats","$longs","$birth_date","$search_preference","$radius_search","$relation_goal","$interest","$language","$religion","$multifile","$profile_bio");
